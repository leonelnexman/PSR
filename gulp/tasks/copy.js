...
import avif from '../util/gulp-avif';
...
gulp.task('copy:compressed', () => gulp
    ...
    .pipe(gulp.src(`${config.src.img}/compressed/*.{jpg,png,jpeg,webp,avif,svg,gif}`))
    .pipe(gulp.dest(config.dest.img))

    .pipe(gulp.src(`${config.src.img}/compressed/*.{jpg,png,jpeg,avif,svg,gif}`))
    .pipe(webp({
        nearLossless: 80
    }))
    .pipe(gulp.dest(config.dest.img))
